...
// Replace the Chat Memo Reference Section through the Program Card display
...
const newSection = `                    {/* === 3-Button Toolbar (チャットメモ / プログラム / 挨拶テンプレ) === */}
                    <div className="border-b border-slate-200 bg-white flex-shrink-0">
                            ...
                            <div className="bg-white p-2 border-b border-slate-200 flex flex-col gap-2 animate-in slide-in-from-top-2 duration-200">
                                ...
                                {isEditingTemplate ? (
                                    ...
                                    <div className="flex gap-2">
                                        <button
                                            type="button"
                                            onClick={() => { handleInsertTemplate(); setActiveToolbarMenu(null); }}
                                            className="flex-1 py-2 bg-tree-600 hover:bg-tree-700 text-white rounded-xl text-xs font-black shadow-sm transition-all active:scale-95 flex items-center justify-center gap-1"
                                        >
                                        ...
                                        <button
                                            ...
                                            className="px-3 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl text-xs font-black shadow-sm transition-all active:scale-95 flex items-center gap-1"
                                        >
                        ...
                        <div className="flex items-center p-2 gap-2">
                            <button
                                type="button"
                                onClick={() => setActiveToolbarMenu(prev => prev === 'memo' ? null : 'memo')}
                                className={\`flex-1 py-2 rounded-xl text-xs font-black shadow-sm border transition-all active:scale-95 flex items-center justify-center gap-1 \${activeToolbarMenu === 'memo' ? 'bg-red-500 text-white border-red-600' : 'bg-white text-red-600 border-red-200'}\`}
                            >
                            ...
                            <button
                                ...
                                className={\`flex-1 py-2 rounded-xl text-xs font-black shadow-sm border transition-all active:scale-95 flex items-center justify-center gap-1 \${activeToolbarMenu === 'program' ? 'bg-wood-500 text-white border-wood-600' : 'bg-white text-wood-700 border-wood-200'}\`}
                            >
                            ...
                            <button
                                type="button"
                                onClick={() => setActiveToolbarMenu(prev => prev === 'template' ? null : 'template')}
                                className={\`flex-1 py-2 rounded-xl text-xs font-black shadow-sm border transition-all active:scale-95 flex items-center justify-center gap-1 \${activeToolbarMenu === 'template' ? 'bg-tree-600 text-white border-tree-700' : 'bg-white text-tree-600 border-tree-200'}\`}
                            >
                                <Sparkles className="w-3.5 h-3.5" />
                                <span>挨拶テンプレ</span>
                            </button>
...
const replacement = c.substring(0, startIdx) + newSection + c.substring(endIdx + endMarker.length);
fs.writeFileSync('src/components/MemoPanel.jsx', replacement);
console.log('Replaced successfully. Total lines:', replacement.split('\n').length);
